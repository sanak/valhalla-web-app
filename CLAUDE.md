...
- **Routing backend**: `getRoutingMode()` in `src/utils/routing-engine.ts` picks between `server` (HTTP to `getValhallaUrl()`) and `wasm` (in-browser WebAssembly). `VITE_ROUTING_MODE` sets the default; the "Routing Engine" settings section (`src/components/settings-panel/engine-settings.tsx`) overrides it via `localStorage['valhalla_routing_mode']` (and the tile source — URL plus gzip flag — via `localStorage['valhalla_tile_source']`). **Every Valhalla request goes through `src/utils/valhalla-client.ts`** — that is the only module that knows which backend is active. Add new actions there, not at call sites.
- **WASM mode**: `src/lib/valhalla-wasm/actor.ts` boots a worker from `public/valhalla-wasm/` (gitignored; populated by `npm run wasm:sync` from a built `wasm-bindings` checkout of the valhalla repo). The upstream `index.mjs` proxy is loaded by dynamic import rather than reimplemented, because its postMessage protocol changes upstream while its API surface does not. Tiles come from `VITE_VALHALLA_TILE_URL` / `VITE_VALHALLA_TILE_URL_GZ` (or their localStorage override), injected as `mjolnir.tile_url` / `mjolnir.tile_url_gz`; valhalla picks tar (range requests) vs per-tile from the `{tilePath}` marker, so the app has no layout enum of its own. Tiles are cached in IDBFS at `/valhalla-cache-<hash of url + gz>`: one database per tile source, because valhalla's `id.txt` pins a cache dir to the `tile_url` that filled it (another URL fails the boot with "Tile URL changed") and gzipped tiles are cached as `.gph.gz`. "Clear tile cache" deletes every `/valhalla-cache*` database. The `/tile` MVT endpoint has no wasm equivalent, so the Tiles tab's Valhalla layers switch is disabled in this mode; Graph age is hidden (replaced by the tile URL under "Tileset") because `tileset_last_modified` reports the IDBFS cache directory's mtime once a `cacheDir` is set, not the tileset's actual build time. The tileset coverage outline drawn on the map (`src/components/map/parts/coverage-area.tsx`, via `useCoverageQuery`) comes from `status.bbox`, which only exists when the server was built with a connectivity map (a bare `tile_dir` server returns no `bbox` at all) — so an absent outline in wasm mode can be expected behavior, not a bug.
...
| `VITE_VALHALLA_TILE_URL`     | Wasm tileset: a tar, or a `…/{tilePath}` per-tile URL (overridable via UI)       |
| `VITE_VALHALLA_TILE_URL_GZ`  | `true` when those tiles are gzipped (`valhalla_build_extract --gzip`)            |
...
- **GitHub Pages, fork only** (`.github/workflows/deploy-pages.yml`): on push to `master` of `sanak/valhalla-web-app`, builds a wasm-only bundle and publishes it to `https://sanak.github.io/valhalla-web-app/`. An `if: github.repository == …` guard keeps it inert upstream. `public/valhalla-wasm/` is gitignored, so the workflow `curl`s the five artifacts from `vars.VALHALLA_WASM_BASE_URL` instead of running `wasm:sync`, writes `.env.production.local` with `VITE_ROUTING_MODE=wasm` and `VITE_VALHALLA_TILE_URL` / `VITE_VALHALLA_TILE_URL_GZ` from `vars.VALHALLA_TILE_URL` / `vars.VALHALLA_TILE_URL_GZ` (the demo serves a `--gzip` tar), rewrites `homepage` from `configure-pages`' `base_url`, and copies `index.html` to `404.html` because Pages has no rewrite rules. Both URLs point at a Cloudflare R2 bucket fronted by a `workers.dev` Worker that answers CORS preflights and byte ranges; the Worker must set `Access-Control-Max-Age` (the tile XHRs are synchronous, so an un-cached preflight doubles routing latency) and return an `ETag` (the `worker.js` code-446 recovery relies on it).
